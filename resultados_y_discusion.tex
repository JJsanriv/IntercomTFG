...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ybar,
            bar width=10pt, % Reducido para 4 barras
            width=\textwidth,
            height=9cm, % Un poco más alto para la leyenda
            legend style={at={(0.5,-0.20)}, % Ajustado para más espacio
                          anchor=north, legend columns=2}, % 2 columnas para la leyenda
            ylabel={Tasa de Datos (kbps)},
            xlabel={Script Ejecutado (Ancho de Banda: 1 Mbps)},
            symbolic x coords={minimal\_video.py, minimal\_video\_fps.py, minimal\_video\_resolution.py},
            xtick=data,
            nodes near coords,
            nodes near coords align={vertical},
            nodes near coords style={font=\tiny, /pgf/number format/.cd,fixed,precision=0}, % Más pequeño y entero
            ymin=0,
            yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=0},
            enlarge x limits=0.25, % Más espacio entre grupos de barras
        ]
        % Audio Enviado
        \addplot coordinates {
            (minimal\_video.py,122)
            (minimal\_video\_fps.py,780)
            (minimal\_video\_resolution.py,995)
        };
        % Audio Recibido
        \addplot coordinates {
            (minimal\_video.py,122)
            (minimal\_video\_fps.py,348)
            (minimal\_video\_resolution.py,0)
        };
        % Vídeo Enviado
        \addplot coordinates {
            (minimal\_video.py,1936)
            (minimal\_video\_fps.py,1066)
            (minimal\_video\_resolution.py,1583)
        };
        % Vídeo Recibido
        \addplot coordinates {
            (minimal\_video.py,610)
            (minimal\_video\_fps.py,572)
            (minimal\_video\_resolution.py,0)
        };
        \legend{Audio Enviado, Audio Recibido, Vídeo Enviado, Vídeo Recibido}
        \end{axis}
    \end{tikzpicture}
    \captionof{figure}{Comparación de datos de audio y vídeo enviados y recibidos a 1 Mbps.}
    \label{fig:comparacion_audiovideo_1mbps}
\end{figure}

\vspace{\baselineskip}

\textbf{Conclusiones Generales para 1 Mbps:}

En conjunto, las pruebas a 1 Mbps demuestran que este ancho de banda es críticamente insuficiente para una comunicación de vídeo y audio mínimamente aceptable con la configuración actual de \textit{InterCom}.
\begin{itemize}
...
Al operar con un ancho de banda de 10 Mbps, el script \texttt{minimal\_video.py} muestra una mejora muy significativa en comparación con el escenario de 1 Mbps. Las estadísticas globales revelan que se intentó enviar aproximadamente 5204 kbps de vídeo y se recibieron 4777 kbps, mientras que para el audio se enviaron 975 kbps y se recibieron 925 kbps. Esto indica que una gran proporción de los datos, tanto de audio como de vídeo, logra llegar a su destino.
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            legend style={at={(0.5,-0.20)}, % Ajustado para más espacio
                          anchor=north, legend columns=2}, % 2 columnas para la leyenda
            ylabel={Tasa de Datos (kbps)},
            xlabel={Script Ejecutado (Ancho de Banda: 10 Mbps)},
            symbolic x coords={minimal\_video.py, minimal\_video\_fps.py, minimal\_video\_resolution.py},
            xtick=data,
            nodes near coords,
            nodes near coords align={vertical},
        ...
        % Audio Enviado
        \addplot coordinates {
            (minimal\_video.py,975)
            (minimal\_video\_fps.py,868) % Ajustado al valor de la salida de 10Mbps para fps
            (minimal\_video\_resolution.py,842) % Ajustado al valor de la salida de 10Mbps para res
        };
        % Audio Recibido
        \addplot coordinates {
            (minimal\_video.py,926) % Ajustado al valor de la salida de 10Mbps
            (minimal\_video\_fps.py,760) % Ajustado al valor de la salida de 10Mbps para fps
            (minimal\_video\_resolution.py,842) % Ajustado al valor de la salida de 10Mbps para res
        };
        % Vídeo Enviado
        \addplot coordinates {
            (minimal\_video.py,5204)
            (minimal\_video\_fps.py,4854) % Ajustado al valor de la salida de 10Mbps para fps
            (minimal\_video\_resolution.py,5479) % Ajustado al valor de la salida de 10Mbps para res
        };
        % Vídeo Recibido
        \addplot coordinates {
            (minimal\_video.py,4777)
            (minimal\_video\_fps.py,4415)
            (minimal\_video\_resolution.py,5153)
        };
        \legend{Audio Enviado, Audio Recibido, Vídeo Enviado, Vídeo Recibido}
        \end{axis}
    \end{tikzpicture}
    \captionof{figure}{Comparación de datos de audio y vídeo enviados y recibidos a 10 Mbps.}
    \label{fig:comparacion_audiovideo_10mbps}
\end{figure}
...
La sección de ``Estadisticas de FPS'' es particularmente interesante. Con un objetivo de 12 FPS, el promedio real alcanzado fue de 4.4 FPS, lo que da una eficiencia del 37.0\%. Aunque es una mejora notable respecto al escenario de 10 Mbps (donde era 2.8 FPS y 23.5% de eficiencia), todavía no se alcanza el objetivo de 12 FPS. Dado que 50 Mbps (50000 kbps) es un ancho de banda significativamente mayor que los ~8000 kbps totales que el programa intenta enviar, la limitación para alcanzar los 12 FPS probablemente ya no reside en la capacidad de la red, sino en otros factores. Estos podrían incluir la capacidad de procesamiento de la cámara para entregar fotogramas a esa tasa, la carga de CPU del sistema que ejecuta el script, o la propia lógica interna del script para capturar y enviar los fotogramas. Es importante destacar que no se reportan mensajes de "Socket bloqueado", lo que refuerza la idea de que la red no está congestionada.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            ylabel={Tasa de Datos (kbps)},
            xlabel={Script Ejecutado (Ancho de Banda: 50 Mbps)},
            symbolic x coords={minimal\_video.py, minimal\_video\_fps.py, minimal\_video\_resolution.py},
            xtick=data,
            nodes near coords,
            nodes near coords align={vertical},
            nodes near coords style={
                font=\tiny,
                /pgf/number format/.cd, % Comienza el bloque de formato de número
                fixed,
                precision=0,           % Termina el bloque de formato de número
                yshift=6pt             % yshift como una opción separada de TikZ para el nodo
            },
            ymin=0,
            ymax=9000, % Ajustar ymax para dar espacio a los números
            yticklabel style={/pgf/number format/fixed, /pgf/number format/precision=0},
            enlarge x limits=0.25,
        ]
        % Audio Enviado
        \addplot coordinates {
            (minimal\_video.py,1038)
            (minimal\_video\_fps.py,906)
            (minimal\_video\_resolution.py,876)
        };
        % Audio Recibido
        \addplot coordinates {
            (minimal\_video.py,965)
            (minimal\_video\_fps.py,769)
            (minimal\_video\_resolution.py,833)
        };
        % Vídeo Enviado
        \addplot coordinates {
            (minimal\_video.py,6968)
            (minimal\_video\_fps.py,7227)
            (minimal\_video\_resolution.py,8140)
        };
        % Vídeo Recibido
        \addplot coordinates {
            (minimal\_video.py,6358)
            (minimal\_video\_fps.py,6364)
            (minimal\_video\_resolution.py,7780)
        };
        \legend{Audio Enviado, Audio Recibido, Vídeo Enviado, Vídeo Recibido}
        \end{axis}
    \end{tikzpicture}
    \captionof{figure}{Comparación de datos de audio y vídeo enviados y recibidos a 50 Mbps.}
    \label{fig:comparacion_audiovideo_50mbps}
\end{figure}

\vspace{\baselineskip}

\textbf{Conclusiones Generales para 50 Mbps:}
